...
export const SIGNAL_LABELS: Record<string, string> = {
  ...
  alarm: "Тревога",
  detection: "Фиксация",
  // Чем именно борт сняли, карта не называет: «перехват» говорит об
  // исходе, а не о средстве. Прежняя подпись «работа ПВО» убрана
...
export const THREAT_LABELS: Record<string, string> = {
  ...
  bek: "БЭК",
  aviation: "Авиация",
  // Пары «закрыто — открыто» со своим отбоем. Подпись пустая нарочно:
  // сигнал («Аэропорт закрыт», «Инфраструктура») уже сказал всё, и
  // «Аэропорт закрыт · аэропорт» читалось бы как заикание.
  infra: "",
  airport: "",
  unknown: "Неизвестно"
};
...
/**
 * Подпись сигнала. Для аэропортов — прямо и по-человечески: владелец
 * попросил писать «аэропорт», а не общее «инфраструктура». Закрытие и
 * открытие — это одна пара, и подпись называет её сторону.
 */
export function signalLabel(signal: string, threat?: string): string {
  if (threat === "airport") {
    if (signal === "infra") return "Аэропорт закрыт";
    if (signal === "allclear") return "Аэропорт открыт";
  }
...
// Типовое слово перед именем зоны: «городской округ Белгород». Справочник
// хранит официальное название целиком, а человеку показывается короткое —
// на телефоне это лишняя строка, и ничего сверх понятного она не несёт.
// Ровно то же правило живёт в pipeline/textnorm.py: API отдаёт имена уже
// короткими, но подписи полигонов приходят из public/data/*.json мимо API,
// и здесь их надо укоротить самим.
//
// Только приставка: «Тимашёвский район» остаётся целиком — там типовое
// слово стоит сзади, и без него имя разваливается.
const ADMIN_HEAD =
  /^(?:городской округ(?: город(?:-курорт)?)?|муниципальный округ(?: город(?:-курорт)?)?|городское поселение|сельское поселение|рабочий пос[её]лок|ЗАТО)\s+/i;

/** Имя зоны так, как его показывают человеку. */
export function shortZoneName(name: string): string {
  if (!name) return name;
  return name.replace(ADMIN_HEAD, "").trim() || name;
}
