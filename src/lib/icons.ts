...
/**
 * Ставить ли значок вообще.
 *
 * Значок означает «здесь». Когда окно пролёта вышло, борт по нашей же
 * физике зону покинул — и двухчасовой значок на карте был бы враньём.
 * След события дальше несёт только гаснущая заливка; значок исчезает.
 */
export function iconVisible(ageMs: number, threat?: string): boolean {
  return ageMs < fadeWindow("district", threat);
}
