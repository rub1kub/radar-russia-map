...
@pytest.mark.parametrize("text,signal", [
    ...
    ("На территории области отменен режим беспилотной опасности", "allclear"),
...
def test_signal_menu_after_allclear_is_not_intercept():
    """«до сигнала «Отбой» - «Сбитие»» — словарь ленты, не перехват.

    Инструкция съедалась только до слова «Отбой», перечисление оставалось,
    и «Сбитие» превращало фиксацию борта над Дорогобужским районом в
    перехват. Совет «не подходите к окнам» при этом не должен перекрывать
    фиксацию тревогой: тревога стоит в очереди сигналов раньше.
    """
    text = ("🔴 Внимание! БПЛА Зафиксирован в воздушном пространстве "
            "Дорогобужского Района!\n"
            "– По возможности, оставайтесь дома и не подходите к окнам.\n"
            "– Оставайтесь в безопасном месте до сигнала «Отбой» - "
            "«Сбитие» - «Ошибка направления»")
    observation = parse(text)
    assert observation.signal_type == "detection"
...
@pytest.mark.parametrize("text", [
    "НАПОМИНАЕМ СНИМАТЬ РАБОТУ ПВО И ПОСЛЕДСТВИЙ ЗАПРЕЩЕНО ‼️‼️‼️",
    "⚠️Напоминаем, что съемка последствий и работа ПВО - СТРОГО ЗАПРЕЩЕНА!!!",
])
def test_filming_ban_is_not_an_event(text):
    """Запрет съёмки держал перехват на весь Краснодарский край."""
    assert parse(text).relevant is False


def test_filming_ban_does_not_hide_real_intercept():
    """Настоящий перехват в соседней фразе переживает вырезание запрета."""
    text = "Сбит БПЛА над Курском. Напоминаем: снимать работу ПВО запрещено!"
    assert parse(text).signal_type == "intercept"


def test_imperative_filming_ban_is_not_an_event():
    """«Работу ПВО не снимать» без слова «запрещено» — тоже запрет."""
    assert parse("РАБОТУ ПВО НЕ СНИМАТЬ И НЕ ВЫКЛАДЫВАТЬ ‼️").relevant is False


def test_imperative_ban_does_not_eat_neighbouring_signal():
    """Телеграфный стиль: запрет и событие в одной фразе через запятую.

    Сегмент запрета ограничен запятой — иначе «Идут сбития, просьба работу
    ПВО не снимать» уходил целиком вместе с настоящими сбитиями.
    """
    text = ("Новороссийск и близлежащие внимание! Идут сбития, "
            "убедительная просьба, работу ПВО и сбития не снимать")
    assert parse(text).signal_type == "intercept"


def test_explosives_warning_is_not_an_impact():
    """«Обломки могут содержать взрывчатку» — памятка, а не взрыв.

    Слово «взрывчатку» давало удар девятого уровня из костромского
    объявления опасности, а ворота удара доставали из остатка «работу
    ПВО» — и опасность на 12 зон становилась перехватом.
    """
    text = ("⚡️ На территории нескольких городов и районов Костромской "
            "области объявлена беспилотная опасность. Не снимайте БПЛА и "
            "работу ПВО и не распространяйте эти кадры в интернете. "
            "Корпусные фрагменты сбитых БПЛА могут содержать взрывчатку, "
            "приближаться к ним запрещено.")
    observation = parse(text)
    assert observation.signal_type == "danger"


def test_repelling_attack_now_is_an_intercept():
    """«Идёт отражение атаки БПЛА» — ПВО работает, другими словами."""
    text = ("В Новороссийске идёт отражение атаки БПЛА. Все службы "
            "приведены в состояние максимальной готовности.")
    assert parse(text).signal_type == "intercept"


@pytest.mark.parametrize("text", [
    "Ну сбития жесткие) Дезинформируйте своих участников дальше",
    "Удачи каналам которые утаивают правду, сбитий над городом физически "
    "не может быть. Т. К ракеты не могут просто так лететь сквозь пво",
    "Логика у вас железная, кинуть материал врагу. Что бы они посмеялись, "
    "обосрали вас в том числе. Сбития, сбития.",
    "Не снимайте работу ПВО. Вы помогаете врагу корректировать следующие "
    "атаки БПЛА. Всех любительских операторов найдут!",
])
def test_channel_polemic_is_not_an_event(text):
    """Спор каналов о том, было ли сбитие, — не сбитие."""
    assert parse(text).relevant is False


def test_mod_recap_for_the_night_is_not_a_live_intercept():
    """«уничтожено … в течение ночи, сообщило Минобороны» — сводка.

    Опубликована днём, а перехват рисовался на час публикации: живая лента
    мониторинга Минобороны не цитирует.
    """
    text = ("🎯 8 украинских БПЛА самолётного типа уничтожено над "
            "Дорогобужским Районом в течение ночи, сообщило Минобороны "
            "России. Спасибо бойцам ПВО!")
    assert parse(text).relevant is False


@pytest.mark.parametrize("text,signal", [
    # Совет укрыться при живой фиксации — фиксация.
    ("Мариуполь, продолжаются фиксации БпЛА над городом Всем в укрытия!",
     "detection"),
    # Объявленную тревогу советы не перекрашивают.
    ("ВОЗДУШНАЯ ТРЕВОГА! Все в укрытие, от окон!", "alarm"),
    ("Курск: воздушная тревога, БПЛА зафиксирован, укройтесь", "alarm"),
])
def test_shelter_advice_does_not_mask_detection(text, signal):
    """«Всем в укрытия» — приписка, а не сигнал: борт уже видят."""
    assert parse(text).signal_type == signal
...
# --- Условие вместо события -------------------------------------------------
#
# «Соблюдайте меры безопасности ПРИ СБИТИЯХ БПЛА» — подпись, стоящая в
# каждом втором сообщении кубанских и крымских лент. Разбор читал её как
# состоявшийся перехват: за неделю 47 сообщений из 53 с этим оборотом
# становились «Перехватом» восьмого уровня. Так над Сочи загорелся значок
# работы ПВО, когда над Сочи ничего не было: событие открылось «Опасностью»,
# а подпись-инструкция подняла его до перехвата при слиянии.

@pytest.mark.parametrize("text", [
    "От Тамани до Сочи\nтревога по БПЛА сохраняется\n"
    "Соблюдайте меры безопасности\nПри сбитиях БПЛА\nКраснодарский край",
    "Анапский район тревога по БПЛА повторно\nПри сбитиях БПЛА меры безопасности",
    "Московская область\nСоблюдаем меры безопасности при работе ПВО!",
    "Керчь и близлежащие\nСоблюдайте меры безопасности\nПри сбитиях БПЛА",
    "В случае сбития БПЛА не подходите к окнам",
])
def test_conditional_intercept_is_not_an_intercept(text):
    assert parse(text).signal_type != "intercept"
...
@pytest.mark.parametrize("text", [
    "Сбит БПЛА над Анапой",
    "Работа ПВО в Ростове",
    "Новороссийск. Продолжаются многочисленные фиксации и сбития",
    # Условие рядом с настоящим сбитием не должно его гасить.
    "Сбит БПЛА над Анапой, при сбитиях не подходите к окнам",
])
def test_real_intercept_survives(text):
    assert parse(text).signal_type == "intercept"


def test_filming_ban_is_not_air_defence_work():
    """«Не снимайте работу ПВО» — запрет на съёмку, а не работа ПВО."""
    observation = parse("Пожалуйста не снимайте работу ПВО, последствия "
                        "падения осколков. Знайте за это можно получить срок.")
    assert observation.signal_type != "intercept"


def test_possible_downings_are_a_warning_not_a_downing():
...
def test_extinguished_fire_is_not_an_intercept():
    """Ликвидируют и пожарные: «возгорание ликвидировано» — не сбитый борт."""
    observation = parse("Возгорание на одном из складов под Воронежем, "
                        "произошедшее вследствие удара БПЛА, ликвидировано "
                        "на площади 16 тыс. кв. м.")
    assert observation.signal_type != "intercept"
...
# --- Аудит 13 августа: сводка МО, благодарности, голое «ПВО», хроника -------

def test_mod_daily_recap_in_nominative_case_is_filtered():
    """«Дежурные средства ПВО перехватили…» — сводка и в именительном падеже.

    Прежняя форма правила знала только творительный («дежурными средствами
    ПВО уничтожено»), и дневная сводка расходилась перехватом по двенадцати
    областям из поля «Регион:».
    """
    observation = parse(
        "📣 ПВО уничтожила 147 БПЛА над Россией\n\n"
        "Дежурные средства ПВО перехватили и уничтожили 147 украинских "
        "беспилотных летательных аппаратов самолетного типа. Это произошло "
        "над рядом российских регионов, Крымом и акваторией Черного моря.")
    assert observation.relevant is False


@pytest.mark.parametrize("text", [
    "📣Пост благодарности.📣\n\nХотелось бы выразить огромную благодарность "
    "от лица жителей Новороссийска подразделениям ПВО. За отражение налета.",
    "Приветствуем наших подписчиков!\n"
    "🙏🏻Выражаем огромную благодарность подразделениям ПВО, ВКС",
])
def test_gratitude_post_is_not_an_event(text):
    """Благодарность расчётам — про налёт, который уже отбит."""
    assert parse(text).relevant is False


def test_air_defence_on_standby_is_not_air_defence_at_work():
    """«Силы ПВО находятся в готовности» — готовность, а не перехват."""
    observation = parse(
        "📣 Обнаружение БПЛА в Воронежской области\n\n"
        "В Каменском районе зафиксирован беспилотный летательный аппарат. "
        "Силы ПВО находятся в готовности.")
    assert observation.signal_type == "detection"


@pytest.mark.parametrize("text", [
    "🚨 Работает ПВО по БПЛА. Не находитесь на пляжах и набережных.",
    "Отработало ПВО по БПЛА. Ожидаем объявление опасности.",
    "Заинский район | Работа ВКС по БПЛА",
    "Алексин | Тульская область | Работа ПВО по БПЛА | Меры безопасности",
])
...
@pytest.mark.parametrize("text", [
    "❗️Число пострадавших при атаке украинских БПЛА на Краснодарский край "
    "выросло до 24, а погибших — до трёх. 12 человек госпитализированы.",
    "🛡 В Новороссийске после атаки БПЛА повреждены более 20 домов",
    "❗️Украинские дроны начали атаковать водопроводы в России. "
    "«В настоящее время прекращена подача воды в Новороссийске»",
])
def test_aftermath_headline_is_not_an_event(text):
    assert parse(text).relevant is False


@pytest.mark.parametrize("text", [
    # ДТП: «их сбил автомобиль» — голое «сбил» перехватом быть не должно.
    "В Мурино под колёсами машины погиб самокатчик\n\n"
    "Мужчина и женщина ехали на самокате, когда их сбил автомобиль.",
    # Призыв в новости: «их необходимо сбивать» — никого ещё не сбили.
    "Эти разведчики не должны свободно летать, их необходимо сбивать!",
])
def test_downing_verb_without_a_drone_nearby_is_not_an_intercept(text):
    assert parse(text).signal_type != "intercept"


def test_governor_recap_of_repelled_attack_is_filtered():
    """«Утром и днем произошла атака. Была успешно отражена» — уже кончилось."""
    observation = parse(
        "Утром и днем в Нижегородской области произошла атака вражеских "
...
def test_channel_digest_gives_each_place_its_own_signal():
    """Лента склеивает десяток сводок — перехват из одной не красит все.

    Подмосковные ленты в налёт публикуют одним сообщением всё сразу:
    «…Чехов Работа ПВО по БПЛА продолжается Раменский район Пролеты
    БПЛА…». Общий сигнал такого текста — максимум по кускам, и перехват
    ложился на девять зон разом. Место берёт сигнал ближайшей сводки.
    """
    from pipeline.parse import phrase_signals

    phrases = [
        "С Белых Столбов в сторону Подольска группа БПЛА",
        "Находимся в укрытиях",
        "Уварово",
        "Летят на Подольск",
        "Чехов",
        "Работа ПВО по БПЛА продолжается",
        "Раменский район",
        "Пролеты БПЛА",
    ]
...
def test_enumeration_of_districts_keeps_the_common_signal():
    """«ГО Подольск, ГО Домодедово… работа ПВО» — один сигнал на всех.

    Перечисление адресатов делить нельзя: сводка в нём одна, и относится
    она ко всем названным округам.
    """
    from pipeline.parse import phrase_signals

    phrases = ["ГО Подольск", "ГО Домодедово", "ГО Бронницы", "ГО Чехов",
               "Московская область", "Продолжается работа ПВО по БПЛА",
               "Меры безопасности"]

    assert phrase_signals(phrases) == {}


def test_short_message_is_never_split():
    """Обычное сообщение из пары фраз разбирается целиком."""
    from pipeline.parse import phrase_signals

    assert phrase_signals(["Керчь", "Сбитие БПЛА"]) == {}


def test_running_total_of_the_raid_is_not_an_intercept():
    """«Число сбитых выросло до 89… всего с начала атаки» — итог, не точка.

    Накопительный счёт со слов мэра ложился перехватом на Москву и
    подмосковные округа.
    """
    text = ("Число сбитых на подлёте к Москве беспилотников выросло до 89. "
...
@pytest.mark.parametrize("text", [
    # Наземная цель: сводка о работе по инфраструктуре, а не борт в небе.
    "Доброполье: пункт управления БПЛА 15-й отдельной бригады особого "
    "назначения нацгвардии Украины уничтожен. По данным МО РФ",
    "Склад беспилотников противника поражён",
    # Доклад о тишине: «уничтожен» рядом не делает его перехватом.
    "В Пензенской области пока без фиксаций, Основной поток пролетел мимо "
    "в направления тыла, или был весь уничтожен. Фиксаций пока не ожидаем.",
    # Идиома: тревога в переносном смысле.
    "Мэр обвинил горожан в топливном кризисе. Эпичный с тревогой ждёт "
    "сентября — из отпусков вернутся все краснодарцы.",
])
def test_texts_that_only_look_like_events(text):
    assert parse(text).relevant is False


def test_future_work_of_air_defence_is_a_warning():
...
def test_expected_attack_with_probable_targets_is_a_forecast():
    """«Ожидаем массовую атаку… предполагаемые цели» — прогноз, не фиксация.

    Между «ожидаем» и словом события помещается определение, и оборот
    проходил мимо: перечень возможных целей становился фиксацией на
    Краснодар и всё Причерноморье.
    """
    text = ("Краснодарский край Напоминаем что мы ожидаем массовую атаку "
            "БПЛА. Предпологаемые цели противника: Причерноморье Таманский "
            "полуостров Славянск-на-Кубани Крымск Ильский Афипский Краснодар")
    observation = parse(text)
...
@pytest.mark.parametrize("text", [
    # Итог по волне за названный словами период.
    "В период со вчерашнего вечера до 5:00 утра сегодня в сторону "
    "Московского региона летели 620 БПЛА, из них 180 беспилотников "
    "уничтожены в Московском регионе.",
    "600 БПЛА летели в сторону Московского региона, из них 201 уничтожен "
    "непосредственно над областью.",
])
def test_running_total_over_a_named_period_is_not_an_intercept(text):
    """«Из них N уничтожены» — счёт по всей волне, а не работа ПВО в точке.

    Прежняя защита ловила интервал только с цифрами («с 20:00 до 7:00»),
    и сводка со словесным началом периода ложилась перехватом на область.
    """
    assert parse(text).relevant is False


def test_live_air_defence_work_survives_the_recap_guard():
    """Живое «работа ПВО» в налёт остаётся перехватом.

    Ночь с шестью сотнями бортов даёт десятки честных одноисточниковых
    перехватов — глушить их вместе со сводками нельзя.
    """
    for text in ("Котельники Работа ПВО Меры безопасности",
                 "ГО Павловский Посад Московская область Сбитие БПЛА",
                 "Бронницы Раменское Воскресенск Чехов Подольск Домодедово "
                 "Московская область Продолжаются фиксации БПЛА "
                 "Продолжается работа ПВО по БПЛА"):
        assert parse(text).signal_type == "intercept"


def test_airport_opening_with_names_in_between_is_an_allclear():
    """«Аэропорт Краснодар и Геленджик открыты» — открытие, а не закрытие.

    Между «аэропорт» и «открыт» помещаются имена, а шаблон требовал их
    подряд: карта писала «аэропорт закрыт» на новость об открытии. За две
    недели корпуса — 21 такое сообщение.
    """
    assert parse("Аэропорт Краснодар и Геленджик открыты").signal_type == "allclear"
    assert parse("Аэропорт Сочи открыт").signal_type == "allclear"
    # Закрытие остаётся закрытием.
    assert parse("Аэропорт Краснодар. Введены временные ограничения").signal_type == "infra"
