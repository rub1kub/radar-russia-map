...
def test_shelter_preparation_news_is_not_a_danger():
    """Подготовка укрытий «на случай» — новость, а не действующая опасность.

    «Управляющие компании готовят подвалы под укрытия… на случай
    объявления опасности» висело «Опасностью» по Краснодару, где в этот
    момент ничего не происходило.
    """
    o = parse("Управляющие компании готовят подвалы под укрытия\n\n"
              "В Краснодаре прошло совещание с УК по подготовке помещений "
              "на случай объявления опасности. Для укрытия жителей могут "
              "использовать подвалы, подземные паркинги, переходы")
    assert not o.relevant
    assert not parse("В школах города подготовка помещений под укрытия "
                     "на случай атак БПЛА").relevant
    # Настоящее оповещение рядом с теми же словами выживает.
    assert parse("Опасность атаки БПЛА! Укройтесь в подвалах "
                 "и паркингах").relevant
    # «ПРИготовиться занять укрытие» — живая команда, не подготовка.
    assert parse("Погар приготовиться занять укрытие! 2 БПЛА Дартс "
                 "в тыл").relevant


def test_prepare_for_fixations_stays_slang_not_alarm():
    """«Приготовиться к фиксациям» — тот же дуплет, что «тревога по БПЛА».

    «Краснодар, тревога по БпЛА / Приготовиться к фиксациям» — типовая
    двустрочная лента: слово «тревога» уже понижено до опасности, но
    «приготовиться к X» без разбора X ловилось само по себе и поднимало
    опасность обратно до объявленной тревоги.
    """
...
def test_calming_words_do_not_cancel_live_air_defence():
    """«ПВО защищает вас, без паники» — работа ПВО, а не отбой.

    «Без паники» лежало в классе отмены безусловно, и успокоение во
    время живого перехвата закрыло его как отбой. Теперь «без паники» и
    «наши ВКС» — отмена лишь тогда, когда после их выреза не остаётся
    живого сигнала.
    """
    assert parse("Краснодар ПВО защищает вас, без паники"
                 ).signal_type == "intercept"
    assert parse("Близлежащие регионы, соблюдайте спокойствие.\n"
                 "Пермский край Сбитие БПЛА, работают наши ВКС"
                 ).signal_type == "intercept"
    # Свой борт и объяснение звука — по-прежнему отмена.
    for text in ("Геленджик! Наш БПЛА в небе, без паники!",
                 "В небе над ЛНР работают наши ВКС",
                 "Ильский\nЗвук исходил от НПЗ. Разминирование, без паники",
                 "Сочи\nБыло зафиксировано землетрясение, 3.8 балла, "
                 "без паники",
                 "Ильский Афипский и близлежащие По уточнению, работы ПВО "
                 "не было. Без паники. Тревог нет"):
        assert parse(text).signal_type == "retracted", text
    # «Уточняем причину объявления» — не отмена: объявление состоялось.
    assert parse("Уточняем причину объявления Ракетной Опасности "
...
def test_no_fixations_observed_is_not_a_fixation():
    ...
    Ловилось только когда тип угрозы вовсе не назван: «фиксаций РАКЕТ не
    наблюдается» называет ту же угрозу, которую отрицает, и осечка
    прежнего чека не гасила её. Заодно расширены сами обороты: «фиксаций
    нет» без «без», «запусков… не наблюдаем» без слова «фиксация», голое
    «ничего не наблюдаем», и окно между подлежащим и «не наблюдаем» шире.
    """
    for text in (
        "По обстановке все спокойно, фиксаций ракет не наблюдается.",
        "Геленджик и Новороссийск пока фиксаций нет, режим внимание",
        "Запусков БПЛА от Харьковской области пока не наблюдаем. "
        "Возможно ночь пройдёт тихо, продолжаем работать.",
        "Погода играет на руку противнику. Облачность мешает при "
        "выявлении бпла. Пока ничего не наблюдаем у нас в области",
        "Фиксаций в направлении Воронежской области и далее в тыловые "
        "регионы не наблюдаем. Ночь должна пройти спокойно",
    ):
        assert not parse(text).relevant, text
    # Настоящее наблюдение рядом с «в регионе тихо» не гасится — вторая
    # половина сообщения не про то же место.
    assert parse("В регионе тихо, над Тульской областью фиксации БПЛА"
                 ).signal_type == "detection"


def test_false_fixation_report_is_retracted():
    """«Ложные фиксации», «ложные срабатывания» — не подтверждённый борт.

    «Ложные срабатывания» не ловилось: класс знал только совершенный вид
    «сработ-» («ложное сработал»), а несовершенный «срабат-» — другая
    основа. Заголовок «Ложные фиксации в Тарасовском районе» и слово
    «зафиксированы» внутри такой новости раньше сами по себе давали
    Фиксацию восьмого уровня, хотя текст прямо говорит «угроза
    отсутствует».
    """
    o = parse("Ложные фиксации в Тарасовском районе\n\n"
              "В Тарасовском районе Ростовской области зафиксированы "
              "ложные срабатывания. Угроза отсутствует.")
    assert o.signal_type == "retracted"


def test_possible_intercept_is_not_a_confirmed_one():
    """«Возможно сбития», «возможна работа ПВО» — предположение, не факт.

    Самоаудит нашёл «Мысхако. Возможно пошли сбития. Не попадите под
    осколки!» и «Сохраняется внимание… Возможна работа ПВО в ваших
    районах!» классифицированными Перехватом — глаголом-связкой между
    «возможно» и словом события («пошли») и другой формой оборота
    («возможна работа ПВО» вместо «возможны сбития») старое правило не
    ловило.
    """
...
def test_hoped_for_intercept_is_not_a_confirmed_one():
    """«Слышны взрывы, надеемся на сбитие» — надежда, а не подтверждение.

    Канал не знает, что упало. Финальное правило удара («снять слово
    „взрыв“, пересмотреть остаток») находило в остатке «сбитие» и само
    поднимало удар до перехвата — «надежда» вырезается ещё на входе,
    вместе с INSTRUCTION_RE/FILMING_BAN_RE, а не после.
    """
    o = parse("Алчевск — слышны взрывы, надеемся на сбитие. ЛНР")
    assert o.signal_type != "intercept"
    # Прилёты по решению владельца не публикуются: без «надежды» в тексте
    # остаётся голое «взрыв», и оно гасится тем же правилом целиком.
    assert not o.relevant
    # Отдельный подтверждённый перехват в другом месте того же сообщения
    # выживает — вырезается только сама фраза надежды.
    assert parse("Стаханов, слышны взрывы. Надеемся на сбитие. Алчевск, "
                 "по предварительным данным сбили БПЛА"
                 ).signal_type == "intercept"


def test_how_to_report_memo_is_not_an_intercept():
    """«Сообщить о работе ПВО можно в этот бот» — памятка, не доклад.

    Список категорий, которые бот принимает («пролёт БПЛА, сирена, работа
    ПВО, громкие взрывы»), содержит слова-события и сам по себе зажигал
    Перехват там, где всё сообщение — реквизиты обратной связи канала.
    """
    for text in (
        "Обратная связь с нами — @RadarRznBot\n\nСообщить о пролёте БПЛА, "
        "сирене, работе ПВО, громких взрывах можно в этот бот.",
        "О всех пролётах, сбитиях и других подозрительных звуках пишите "
        "в сообщения каналу или нашим администраторам: Ваша бдительность "
        "помогает! И убедительная просьба — не публиковать видео о "
        "пролётах и сбитиях в открытых источниках!",
    ):
        assert not parse(text).relevant, text
...
def test_planned_siren_test_is_not_an_alarm():
    """Плановая проверка сирен не должна зажигать тревогу на карте."""
    messages = (
        "Краснодар До конца недели будут слышны одиночные сирены, "
        "плановая проверка. Просьба не беспокоиться, все под контролем. "
        "В случае опасности оповестим!",
        "В Советском районе Рязани пройдет плановая проверка системы "
        "оповещения с включением сирен. Угрозы для населения нет.",
        "В Сызрани пройдёт плановая проверка локальной системы оповещения. "
        "С 12:00 до 13:00 включат электросирены.",
        "В Сызрани запланировано проведение проверки ЛСО с запуском "
        "сиренных установок и доведением проверочного сообщения.",
        "Ильский НПЗ 15:30 проверка сирен.",
        "С 13:00 в городе пройдёт учебная проверка системы оповещения. "
        "Сирены могут быть слышны в центре.",
        "На пунктах установки сирен будут проводиться обслуживание и тесты. "
        "Диктор сообщит: проводится проверка системы оповещения.",
        "Уважаемые краснодарцы. Периодически слышны одиночные звуки сирен. "
        "Просим не беспокоиться — это плановые проверки. Специалисты "
        "проверяют работу систем оповещения. Одиночные запуски продолжатся "
        "до конца недели.",
        "В Тюмени проведут тренировку системы оповещения. При угрозе БПЛА "
        "сирены в дальнейшем будут включать для передачи сигнала.",
    )
...
def test_week_ahead_forecast_is_not_an_event():
    """«Готовит массовые пуски в течение этой недели» — агитка, не налёт.

    Прогноз с горизонтом в дни рисовал «Тревогу · групповой налёт» на
    регион. Конкретное сегодняшнее окно рядом сообщение спасает: ночное
    предупреждение — настоящее.
    """
    assert not parse("Враг готовит массовые пуски БПЛА по всем "
                     "направлениям в течение этой недели. "
                     "Не игнорируйте сигналы тревог").relevant
    assert not parse("В ближайшие дни планируется массовая атака "
                     "1- НПЗ 2- порты").relevant
    assert parse("По предварительной информации, сегодня 02:00 по 06:00 "
                 "и в ближайшие дни возможны пуски ракет в тыловые "
                 "регионы").relevant


def test_night_audit_aftermath_and_backdrop_are_not_events():
    """Аудит ночи 24.08: хроника последствий не рисует значков.

    «БПЛА попал в детский центр, двое детей погибли» шло Фиксацией,
    «до трёх увеличилось количество погибших» — Опасностью, «жители на
    фоне атаки ночуют в коридорах» — Тревогой, «ночь была тяжелая, не
    игнорируйте сирены» — Тревогой. Всё это рассказ после факта.
    """
    for text in (
        "БПЛА ВСУ попал в детский центр в Краснодаре, двое детей "
        "погибли, ещё семь пострадали.",
        "Под атаку БПЛА попал логистический центр Ozon в Краснодаре. "
        "Предварительно, пострадавших нет",
        "До трёх увеличилось количество детей, погибших при атаке БПЛА "
        "на Краснодар.",
        "Жители Краснодара на фоне атаки БПЛА ночуют в коридорах и "
        "ванных.\n\nМестные сообщают, что проводят ночь в коридорах",
        "Мальчики и девочки, ночь была сегодня тяжелая, и к сожалению "
        "есть погибшие среди гражданских людей.\n\nПожалуйста не "
        "игнорируйте сирены",
    ):
        assert not parse(text).relevant, text
    ...
    # «Не попадите под атаку» — предупреждение, не фиксация.
    assert parse("Туапсе - не попадите под атаку БПЛА."
                 ).signal_type == "danger"
...
def test_broad_audit_service_phrases_do_not_shift_classes():
    ...
    «Силы ПВО приведены в повышенную готовность» поднимало фиксацию до
    тревоги; «вчера сняла работу ПВО, задержана» давало перехват; «всё
    без фейков и паники» губернаторского поста закрывало события как
    отмена; бензиновая сводка «Роснефть, около аэропорта — очередь»
    рождала событие инфраструктуры.
    """
    o = parse("📣 Фиксация БПЛА в Нижнекамске\n\nВ Нижнекамске зафиксирован "
              "беспилотный летательный аппарат. Силы ПВО и экстренные "
              "службы приведены в повышенную готовность.")
    assert o.signal_type == "detection"
    assert not parse("Вчера сняла работу ПВО, сегодня уже извиняется на "
                     "камеру. Жительница Новороссийска была задержана "
                     "сотрудниками полиции").relevant
    assert not parse("Враг пытается нас запугать, но Севастополь становится "
                     "только сильнее. Всё без фейков и паники.").relevant
    assert not parse("❗️Оперативная информация по наличию бензина и "
                     "очередей в Сочи:\n✅ Роснефть, около аэропорта - "
...
def test_airport_closure_survives_notam_footnote_and_attention_word():
    """Официальные закрытия не гасятся сносками и словом «Внимание!».

    NOTAM-сноска «принимает рейсы с 09:00 до 23:00» попадала под
    recap-интервал «с X до Y» и убивала закрытие Краснодара целиком;
    «Внимание!» в объявлении Ижевска выигрывало у инфраструктуры как
    caution — закрытия Ижевска не существовали для карты вовсе.
    """
    krasnodar = parse(
        "▫️Аэропорт КРАСНОДАР (Пашковский)\n\n"
        "✈️ВВЕДЕНЫ дополнительные* временные ограничения на прием и "
        "выпуск воздушных судов.\n\n"
        "*Согласно действующему NOTAM, аэропорт Краснодар принимает "
        "регулярные рейсы с 09:00 до 23:00.")
    assert (krasnodar.signal_type, krasnodar.threat_type) == (
        "infra", "airport")
    izhevsk = parse(
        "Внимание! В 06:20 31 июля на территории международного "
        "аэропорта Ижевска имени М. Т. Калашникова введены временные "
        "ограничения на прием и выпуск воздушных судов.")
    assert (izhevsk.signal_type, izhevsk.threat_type) == ("infra", "airport")


def test_war_correspondent_bragging_is_not_our_airspace():
    """Военкорский дайджест чужого фронта — не события у нас.

    «Дроноводы сожгли технику ВСУ в районе Дружковки», «три бригады ВСУ
    попали под раздачу, уничтожены пункты дислокации», «FPV-перехватчики
    устроили разборки» — хвастовство о той стороне шло фиксациями и
    перехватами по домашнему региону канала (94% тяжёлых событий
    rostovskoe_nebo_radar никто не подтверждал именно поэтому).
    """
    for text in (
        "💥 FPV-перехватчики «Центра» устроили разборки с вражескими "
        "БПЛА в небе.",
        "🔥 Дроноводы сожгли автомобильную технику ВСУ в районе Дружковки",
        "✈️ Сразу три бригады ВСУ попали под раздачу нашей авиации. "
        "Точными ударами уничтожены: пункт временной дислокации",
    ):
        assert not parse(text).relevant, text
    # Живой перехват у нас не задет.
    assert parse("Работа ПВО в Ростове, сбит БПЛА").signal_type == "intercept"
    assert parse("Таганрог отражает атаку БПЛА. Идёт отражение атаки"
                 ).signal_type == "intercept"


def test_announcement_with_wait_for_allclear_is_not_an_allclear():
    """«Ждите отмены сигнала опасности» — инструкция, а не отмена.

    Объявление мэрии о НАЧАЛЕ тревоги («звучит сигнал „Внимание всем" —
    идёт отражение атаки БПЛА… ждите отмены сигнала опасности») ловилось
    классом отбоя — «отмены сигнала опасности» совпадало с отбойной
    формой — и закрывало все открытые события Краснодара тремя «Отбоями»
    подписчику в три часа ночи. Туда же «сирена перестала звучать — это
    НЕ значит, что тревога отменена»: отрицание читалось отменой.
    """
    mayor = parse("📢 Внимание! В Краснодаре звучит сигнал «Внимание всем» "
                  "– идёт отражение атаки БПЛА\n\n"
                  "🔴 Ждите отмены сигнала опасности и следите за новостями")
    assert mayor.signal_type == "intercept"
    negated = parse("Если сирена перестала звучать - это не значит, что "
                    "тревога отменена. Идет отражение атаки БПЛА")
    assert negated.signal_type == "intercept"
    # Настоящие отбои не задеты.
    for text in ("Отбой опасности по БПЛА",
                 "Отмена сигнала беспилотной опасности",
                 "Отбой воздушной тревоги, можно выходить из укрытий"):
        assert parse(text).signal_type == "allclear", text
