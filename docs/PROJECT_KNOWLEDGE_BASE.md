...
Проект работает в production на `https://tihoenebo.com`:

- React и OpenLayers отображают живую карту, ленту, историю и аналитику.
- Тёмная и светлая подложки — label-free векторные стили OpenFreeMap;
  спутник и hillshade предоставляет Esri.
- Python-конвейер собирает открытые Telegram-источники, разбирает и сливает
  наблюдения в SQLite; FastAPI отдаёт REST/WebSocket.
- Vite собирает клиент, серверные скрипты ежечасно обновляют SEO-страницы,
  маршруты и сводки.
- Карта, API, Telegram/Web Push, SEO и фоновые таймеры развернуты на одном
  Ubuntu-хосте; архитектура подробно описана в `docs/AI_HANDOFF.md`.
...
- резкая векторная подложка OpenFreeMap без provider-подписей и отдельный
  hillshade Esri;
...
- собственный tile server отсутствует: runtime зависит от OpenFreeMap и Esri;
- локальные тематические Natural Earth-слои остаются обзорными;
- гарантированная официальная нормализация каждого малого населённого пункта
  по ФИАС/ОКТМО не завершена;
- внешнего CI нет, проверки и deployment выполняет `scripts/deploy.sh`;
- юридические условия внешних tile providers требуют периодического аудита.
...
| Подложка | Векторные стили OpenFreeMap без provider `symbol`-слоёв |
...
TypeScript работает в strict mode, `noEmit` включен. Отдельного линтера и
formatter-конфигурации нет. Python-контур проверяется через `pytest`, frontend —
через `Vitest`; общий минимальный прогон собран в `./scripts/check.sh`.
...
flowchart LR
    ...
    G["OpenFreeMap vector tiles"] --> F
    I["Esri hillshade tiles"] --> F
    T["Telegram sources"] --> P["ingest + pipeline"]
    P --> S["SQLite"]
...
На production работают systemd-сервисы сбора, парсинга и API, SQLite-база,
ежечасная генерация SEO/маршрутов и Apache. Браузер получает HTML/CSS/JS,
геоданные и `/api/*` с нашего хоста; запросы к внешним tile providers делает
напрямую. Точные unit names, порты и порядок deployment — в `AI_HANDOFF.md`.

### 7.2. Что выполняется в браузере

Браузер:

1. Загружает `regions.json`, `districts.json`, обзорные места и live-state.
2. Парсит GeoJSON регионов и районов в EPSG:3857.
3. Создаёт слои событий, регионов, районов и обзорных подписей.
4. Строит поисковый каталог регионов, районов и населённых пунктов.
5. Создаёт векторную подложку, hillshade и тематические слои.
6. Подгружает тяжёлые JSON и 2°-ячейки малых мест только по слою/масштабу/окну.

Полные 196 тыс. малых мест больше не создаются одним массивом на старте; см.
`place-labels/manifest.json` и секцию про ячеистую загрузку в `AI_HANDOFF.md`.
...
`src/lib/vectorBasemap.ts` удаляет из стиля raster relief и все `symbol`-слои,
после чего `ol-mapbox-style` рисует только векторную геометрию. Русские подписи
приходят из собственных слоёв. Hillshade включается вместе с подложкой, но не
показывается на спутнике. Отдельного пользовательского переключателя рельефа нет.

Поддерживаемые build-time переменные:

```text
VITE_BASEMAP_DARK_STYLE_URL
VITE_BASEMAP_LIGHT_STYLE_URL
VITE_HILLSHADE_URL
...
|---:|---|---|---|---|
| 0 | OpenFreeMap vector basemap | включен | внешний MVT/style | сразу |
...
| 26 | Городские контуры | выключен | `urban-areas.json` | по включению, zoom `>= 4.3` |
| 20 | Районы | включен | `districts.json` | сразу, layer minZoom `4.15` |
| 23 | Железные дороги | выключен | `railways.json` | по включению, zoom `>= 4.8` |
| 24 | Дороги | выключен | `roads.json` | по включению, zoom `>= 4.65` |
| 28 | Регионы | включен | `regions.json` | сразу |
| 30 | Обзорные подписи мест | включен | `city-labels.json` | сразу, ярусы по населению/типу |
| 31 | Детальные подписи мест | включен | `place-labels/*` | ячейки окна с zoom `>= 9` |
...
Малые места показываются с близких масштабов через ячейки
`public/data/place-labels/`, поэтому не требуют загрузки всего справочника.
...
- `nameLocked` сохраняется у всех 89 supplemental-районов и запрещает live-
  gazetteer заменять закреплённое русское имя; сам gazetteer использует
  опубликованный `properties.zone`, поэтому смена подписи не меняет подписки,
  SEO и API;
...
| OpenFreeMap / OpenMapTiles / OSM | runtime vector basemap | условия провайдера и ODbL attribution | внешний сервис без SLA; долгосрочно нужен fallback/self-hosting |
| Esri | runtime hillshade и спутник | условия провайдера | нужен production-тариф/SLA или собственный fallback |
...
У supplemental ADM2 есть известная геометрическая ошибка вокруг Севастополя:
один полигон захватывает часть Крыма. В справочнике поэтому существуют
логические Балаклавский, Гагаринский и Ленинский районы без выдуманного
полигона; Фиолент закреплён за Балаклавским по `source_id`, Севастополь
подчинён федеральному региону напрямую, Евпатория — Республике Крым.

Геокодер раскрывает общий суффикс «Саратовской/Пензенской областей», но
обороты «стык/граница» использует только как контекст конкретного НП.
Смешанный отбой обрезает географию перед новым сохраняющимся статусом, а
`Fuser` принимает один `(raw, zone, signal, threat)` только один раз: два
топонима, укрупнившиеся в один район, не создают событие-близнец.

Всего массовая канонизация исправляет 1 479 имен. Ещё 25 русских названий
(например, `Ртищево`, `Правый Берег`, `Киевский`, `Каменнобродский`)
закреплены по GeoNames ID: городские части `PPLX` не входят в ОКТМО как
самостоятельные НП, а у Ртищево массовый выбор брал ошибочную форму.
Исторические alternate names автоматически не принимаются. Итого относительно
старой сборки исправлено 1 504 отображаемых
имени. Один
...
Текущая карта показывает attribution OpenFreeMap/OpenMapTiles/OSM и Esri.
