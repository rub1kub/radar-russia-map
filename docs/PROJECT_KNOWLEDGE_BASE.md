...
Статус документа: канонический источник знаний по текущей рабочей копии.  
...
| Росстат ОКТМО | пока только подсчет строк в summary | официальные открытые данные; условия проверить перед публикацией | в runtime-объекты не присоединен |
...
Срез после `prepare:data` 26.07.2026:

| Файл | Объекты | Raw | Gzip локальной оценки | Загружается |
|---|---:|---:|---:|---|
| `regions.json` | 89 | 5.35 MiB | 1.39 MiB | сразу |
| `districts.json` | 2 327 | 14.42 MiB | 3.76 MiB | сразу |
| `places.json` | 209 789 | 21.08 MiB | 3.96 MiB | сразу |
...
- все 2 327 объектов получены только из geoBoundaries RUS ADM2;
- все `iso` сейчас `null`;
- `regionId` или родительское имя не сохраняется;
- supplemental-территории районными полигонами не покрыты;
- 102 названия встречаются более одного раза в разных местах;
- поиск и details не могут различить одноименные районы по региону.
...
- 26 546 имен содержат латиницу;
- 26 540 имен вообще не содержат русской кириллицы;
- украинских букв `ІЇЄҐ` после текущей нормализации не найдено;
...
1. GeoNames RU загружается из ZIP в память.
2. Из административных объектов и населенных пунктов строится lookup русских имен.
3. Формируются варианты `District`, `Rayon`, `Municipality`, `Urban Okrug`,
   `Gorodskoy Okrug` и формы прилагательных.
4. Кандидаты ранжируются по типу и расстоянию Левенштейна.
5. Если совпадения нет, используется собственная латиница -> кириллица.
6. Применяется ручной `districtNameOverrides`.

Текущие ручные исправления включают Ростов-на-Дону, Верхний Уфалей и несколько
обрезанных названий городских округов.

Населенные пункты RU:

- исходное кириллическое имя имеет приоритет;
- иначе выбирается ближайшая русская alternate name;
- для 843 координат имя может быть заменено именем из RadarMap reference.
...
Отсутствие украинских букв не означает корректность всех русских топонимов.
Автоматические морфологические замены могут создавать неверные формы.
...
Важно: даже чистая правка CSS при новом запуске dev заново выполняет тяжелый ETL.
Для ежедневной разработки стоит разделить `dev` и `prepare:data`.
...
2. Проверить GeoNames candidates.
3. Добавить узкий exact override только при необходимости.
4. Пересобрать.
5. Проверить отсутствие латиницы и ошибочного совпадения в другом регионе.
