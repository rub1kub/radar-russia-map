...
# Через сколько событие перестаёт красить зону в свой цвет.
#
# Два часа, а не три: карта показывает, что происходит, а не что случалось.
# Час без единого нового сообщения — это уже прошлое, и гореть оно должно
# еле-еле.
ZONE_FADE = timedelta(hours=2)
...
def zone_fade(last_seen: str, now: datetime) -> float:
    ...
    Кривая крутая в начале и пологая в конце. Линейная убыль оказалась
    слишком щадящей: за пятьдесят минут без единого нового сообщения район
    терял всего четверть яркости и выглядел как горящий сейчас. Человеку
    важна разница между «пять минут назад» и «час назад», а между «два
    часа» и «три» — уже нет.
    ...
    if age <= 0:
        return 1.0
    share = min(1.0, age / ZONE_FADE.total_seconds())
