...
# Единственный класс, который поднимается с региона на подписку города:
# объявленная воздушная тревога (и её отбой — у события отбоя тот же
# signal_type). Сирена звучит на весь край и касается каждого города в
# нём — ровно её пропуск 20.08 и был жалобой владельца. Всё остальное —
# опасности, перехваты, фиксации, взрывы, инфраструктура — с краевой
# привязкой до подписки города не доходит: решение владельца 23.08
# («подписался на Краснодар — а мне приходят события с края»).
ANNOUNCED_SIGNALS = {"alarm"}
...
def matches_watch(connection: sqlite3.Connection, zones: set[str],
    ...
    цепочку родителей отслеживаемой зоны. Вверх поднимается только
    объявленная воздушная тревога (сирена): всё остальное с краевой
    привязкой — не событие в твоём городе, и ни «работает ПВО ·
    Краснодарский край», ни «опасность по краю» подписчику города не
    приходят.
