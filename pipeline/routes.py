...
# а не траекторию.
#
# Голые «на Краснодар» и «к Краснодару» — тот же путь без глагола, и это
# самая частая форма в лентах: «Калининский район на Краснодар БПЛА».
# Без них линия не рисовалась там, где источник сам назвал и откуда, и
# куда, — а на город назначения при этом вешалась фиксация. Условия те
# же, что у MOVING_TO_RE в parse: заглавная буква следом (иначе «на юг»
# и «на оптоволокне») и не речной хвост имени («Славянск на Кубани»).
ROUTE_MARKER_RE = re.compile(
    ...
    # «Пролёты БПЛА в Азовское море» — путь, хотя предлог «в»: винительный
    # падеж означает движение. Предложный — место, и маршрутом не является:
    # «Уничтожение БПЛА в акваториИ Чёрного моря» и «в АзовскОМ море»
    # говорят, ГДЕ сбили, а не куда летели. На этом рисовалась стрелка
    # «Новороссийск → Чёрное море» — ровно наоборот тому, как летают.
