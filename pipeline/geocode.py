...
AMBIGUOUS = {
    ...
    # Аэропорты, названные в честь далёких мест: ленты пишут «Аэропорт
    # САРАТОВ (Гагарин)», «ЕКАТЕРИНБУРГ (Кольцово)», а точный индекс знал
    # только город Гагарин под Смоленском (600 км) и наукоград Кольцово
    # под Новосибирском (1600 км). Настоящие тёзки остаются: свои
    # оповещения всегда называют регион или район.
    "гагарин", "кольцово",
}
...
STOPWORDS = {
    ...
    # Степень угрозы из шаблонных лент. Само поле снимается разбором, но
    # каналы пишут её и без слова «уровень» — а посёлки Высокий и Средний
    # есть в двадцати регионах сразу.
    "высокий", "средний",
}

# Ключи сравнения приходят из norm_key, где «э» и «ё» сведены к «е»:
# слово «Аэропорт» превращается в «аеропорт» и мимо сырого списка проходило —
# посёлки Аэропорт под Оренбургом и Тарой собирали каждое «аэропорт ввёл
# ограничения». Поэтому сами списки прогоняются через ту же нормализацию.
STOPWORDS = {norm_key(word) for word in STOPWORDS}
AMBIGUOUS = {norm_key(word) for word in AMBIGUOUS}
...
FEATURE_HEADS = {
    ...
    "водохранилище", "часть", "направление", "тэс", "аэс", "гэс",
    # «По Московской трассе от Гвардейское на Симферополь» — трасса в
    # Крыму, а событие уезжало в Московскую область.
    "трасса", "шоссе", "автодорога",
    # НПЗ здесь namеренно НЕТ: заводы носят имена своих посёлков
    # («Ильский НПЗ», «Афипский НПЗ»), и гашение прилагательного перед
    # «НПЗ» стирало само место — пожар на Ильском НПЗ падал на домашний
    # регион канала-пересказчика, вплоть до Астраханской области.
    # Арабатская Стрелка — коса от Геническа до Крыма, и в сводках она
    # упоминается чаще, чем оба одноимённых посёлка вместе взятые.
    "стрелка",
}
# Та же нормализация, что и у STOPWORDS: «тэс», «аэс», «гэс» в ключах
# выглядят как «тес», «аес», «гес».
FEATURE_HEADS = {norm_key(word) for word in FEATURE_HEADS}
...
# Псевдонимы, которые вытесняют одноимённые НП из справочника целиком.
# «Ракетная опасность на Кубани» — всегда край, но точный индекс знал
# только посёлок Кубань в Гулькевичском районе, и краевые оповещения
# оседали на нём. Сам посёлок в сводках не встречается ни разу; деревни
# Пулково под Ломоносовым в сводках тоже никто не назовёт без аэропорта.
EXCLUSIVE_ALIASES = {"кубань", "пулково"}
...
FEATURE_ALIASES = {
    ...
    # Крупнейшие аэропорты страны не геокодировались вовсе: «Шереметьево»
    # в справочнике нет ни под каким именем, а «Пулково» — безвестные
    # деревни Ломоносовского района, которые разбор отбрасывал. Ограничения
    # в аэропорту ложатся на округ, где он стоит.
    "шереметьево": "городской округ химки",
    "пулково": "санкт-петербург",
    # Кубанские ленты сокращают Славянск-на-Кубани до «СНК»: «СНК сирена»
    # в ночь 24.08 не геокодировалось и падало на весь край.
...
# Соледар» разрешалось в посёлок Артемовск Курагинского района
# Красноярского края — единственного тёзку в точном индексе.
#
# Здесь только те города, у которых цель в справочнике однозначна, а
# российский тёзка отсутствует или безвестен. Северск, Кировск,
# Красноармейск и Попасная сюда намеренно НЕ входят: у первых трёх есть
# крупные российские города-тёзки (Томская, Мурманская, Московская
...
# оттуда, и только там такое имя означает город, а не тёзку.
NEW_REGION_NAMES = {
...
#
# Ленты объявляют опасность не только по районам, но и по «Приазовью»,
# «приазовскому побережью», «Причерноморью». В справочнике таких зон нет —
# это физическая география, а не администрация, — и сообщение либо ложилось
# на весь субъект целиком, либо терялось.
#
# Состав групп зависит от региона: «Запорожская область, Приазовье» — это
# южные округа Запорожья, «Краснодарский край, приазовское побережье» —
# азовские районы Кубани, а в Ростовской области Приазовье своё. Поэтому
# ключ здесь — пара (регион, группа), и разворачивается она, только когда
# регион известен: назван в самом сообщении или это регион канала. Без
# региона имя остаётся неразрешённым, как и раньше: гадать, чьё Приазовье
# имелось в виду, мы не вправе.
#
# Границы взяты по географии Азовского и Черноморского побережий:
# Кубанское Приазовье — Ейский, Щербиновский, Приморско-Ахтарский,
# Славянский, Темрюкский, Каневской районы; Российское Приазовье
# Ростовской области — Азовский, Неклиновский, Мясниковский районы,
# Таганрог и Азов; Черноморское побережье Кубани — Анапа, Новороссийск,
# Геленджик, Туапсинский район, Сочи.
GROUP_ALIASES: dict[tuple[str, str], tuple[str, ...]] = {
    ("краснодарский край", "приазовье"): (
        "ейский район", "щербиновский район", "приморско-ахтарский район",
        "славянский район", "темрюкский район", "каневской район",
    ),
    ("краснодарский край", "причерноморье"): (
        "анапа", "новороссийск", "городской округ геленджик",
        "туапсинский район", "сочи",
    ),
    ("ростовская область", "приазовье"): (
        "азовский район", "неклиновский район", "мясниковский район",
        "таганрог",
    ),
...
# Как эти группы называют в лентах. Ключ — нормализованная фраза, значение —
# имя группы в GROUP_ALIASES.
GROUP_PHRASES = {
    "приазовье": "приазовье",
    "приазовское побережье": "приазовье",
    "азовское побережье": "приазовье",
    "побережье азовского моря": "приазовье",
    "причерноморье": "причерноморье",
    "черноморское побережье": "причерноморье",
    "побережье черного моря": "причерноморье",
}
...
# Короткие псевдонимы пропускает страж длины в _lookup — и региональные
# («ДНР»), и ориентиры («СНК»): без этого трёхбуквенный ключ отбрасывался
# как мусор ещё до поиска.
SHORT_KEYS = {key for key in (*REGION_ALIASES, *FEATURE_ALIASES)
              if len(key) < 4}
...
class Geocoder:
    ...
    def _add_aliases(self, name_sets: dict[str, set[str]],
        ...
        def new_region_city(zone: dict) -> bool:
            # Псевдоним города обязан вести внутрь нового региона: только там
            # сетка округов, из-за которой города и не оказалось в имени.
            root = self.chain(zone["id"])[-1]
            return self.zones[root]["name_ru"].strip().lower() in NEW_REGION_NAMES

        for aliases, accept in ((REGION_ALIASES, region), (FEATURE_ALIASES, landmark),
                                (CITY_ALIASES, new_region_city)):
            for alias, target in aliases.items():
                ...
                if not hits:
                    continue
                if alias in EXCLUSIVE_ALIASES:
                    name_sets[key] = set(hits)
                    stem_sets[stem_key(key)] = set(hits)
                    continue
    ...
    def resolve(self, phrases: list[str],
        ...
        resolved.extend(self._groups(phrases, resolved, home, used))
        return resolved

    def _groups(self, phrases: list[str], resolved: list[Resolved],
                home: str | None, used: set[str]) -> list[Resolved]:
        """Развернуть «Приазовье» и подобное в районы названного региона.

        Регион берётся из самого сообщения, а если там его нет — из прописки
        канала. Без региона группа не разворачивается: чьё именно Приазовье
        имелось в виду — кубанское, ростовское или запорожское — из текста
        не следует, а гадать нельзя.
        """
        text = norm_key(expand_units(" . ".join(phrases)))
        wanted = {name for phrase, name in GROUP_PHRASES.items() if phrase in text}
        if not wanted:
            return []

        regions = [self.zones[item.zone_id]["name_ru"].strip().lower()
                   for item in resolved
                   if self.zones[self.chain(item.zone_id)[-1]]["level"] == "region"
                   or item.level != "region"]
        # Регион каждой найденной зоны плюс регион канала — по порядку
        # надёжности: названное в тексте главнее прописки.
        named = [self.zones[self.chain(item.zone_id)[-1]]["name_ru"].strip().lower()
                 for item in resolved]
        if home in self.zones:
            named.append(self.zones[self.chain(home)[-1]]["name_ru"].strip().lower())

        out: list[Resolved] = []
        for group in wanted:
            for region in dict.fromkeys(named):
                members = GROUP_ALIASES.get((region, group))
                if not members:
                    continue
                for member in members:
                    for zone_id in self.by_name.get(member, ()):
                        zone = self.zones[zone_id]
                        if zone["level"] == "place":
                            continue
                        root = self.chain(zone_id)[-1]
                        if self.zones[root]["name_ru"].strip().lower() != region:
                            continue
                        if zone_id in used:
                            continue
                        used.add(zone_id)
                        out.append(Resolved(zone_id, zone["level"], zone["name_ru"],
                                            zone["lat"], zone["lon"], group))
                break
        return out
    ...
    def _pick(self, match: Match, context: set[str],
        ...
        for zone_id in candidates:
            ...
            in_home = bool(home and set(self.chain(zone_id)) & home)
            ...
            #
            # Исключение — деревня в регионе самого канала, названная точной
            # формой. У «Городища» 114 тёзок, четыре из них в Брянской области,
            # и все четыре — деревни: порог отсекал их раньше, чем учитывался
            # дом, и брянское «Городище ФПВ» уезжало в Волгоградскую область,
            # к самому крупному однофамильцу. Обиходных слов послабление не
            # касается: они отсеиваются выше, по собственному правилу.
            if (single and zone["level"] == "place" and not match.marked
                    ...
                    and not (in_home and match.exact)):
                continue
