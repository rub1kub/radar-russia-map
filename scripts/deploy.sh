...
# Карта /marshruty/ собирается отдельным бандлом с фиксированными именами:
# страницу генерирует сервер, и хеши манифеста ему недоступны.
npx vite build --config vite.routes.config.ts 2>&1 | tail -2
