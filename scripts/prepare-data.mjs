...
const canonicalPlaceNameOverridesById = new Map([
  // GeoNames хранит корректное русское имя среди альтернатив, но старый
  // эвристический выбор брал форму «Ртишчево». ID относится к городу в
  // Саратовской области, а не к одноимённым деревням.
  ["500886", "Ртищево"],
...
// Русские подписи городов для карты. Подложка используется без
// собственных подписей (латиница CARTO на русской карте обстановки
// выглядела чужой), поэтому ориентиры даёт этот слой: имя, точка,
// население и код GeoNames — ярусы появления считает клиент.
//
