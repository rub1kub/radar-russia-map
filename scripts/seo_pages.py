...
def faq_block(name: str, stats: dict | None,
    ...
    qa = [
        ...
        # Люди ищут «карту дронов», а слова «дрон» нет ни у одного
        # конкурента в выдаче — Яндекс под каждым результатом честно пишет
        # «Не найдено: дронов». Вопрос отвечает ровно на этот запрос.
        (f"Где сейчас летают дроны {where}?",
         f"Карта показывает, из каких мест {where} приходят сообщения о "
         f"беспилотниках, и в какую сторону они шли, если источник это "
         f"назвал. Дрон и беспилотник — одно и то же: ленты пишут и так, и "
         f"так. Точного маршрута карта не знает: она показывает места, "
         f"которые назвали источники, и время сообщений."),
...
def page(name: str, slug: str, districts: list, stats: dict | None,
    ...
    # Заголовок собран по самой выдаче, а не по догадкам. У всех, кто стоит
    # выше нас, он начинается словом «Карта» и содержит «воздушная тревога»
    # и «онлайн»: «Карта БПЛА и воздушной тревоги», «Карта БПЛА и тревог по
    # России онлайн», «Радар БПЛА и карта угроз: Московская область онлайн».
    ...
    heading = (f"Карта БПЛА и воздушной тревоги {where}"
               if path_prefix == "region"
               else f"Карта БПЛА и тревог {where}")
    count = stats["events"] if stats else 0
    alias_note = f" ({', '.join(aliases)})" if aliases else ""
    # «Дроны», «работа ПВО», «в реальном времени» — слова, которыми люди
    # ищут. Первого из них нет ни у одного конкурента в выдаче: Яндекс на
    # запрос «карта дронов московская область» честно пишет «Не найдено:
    # дронов» под каждым результатом.
    if count:
        description = (
...
<html lang="ru">
  ...
  <body>
    <main>
      ...
      <p>
        Воздушные тревоги, сообщения о беспилотниках и дронах, работа ПВО,
        предупреждения об опасности и отбои — так, как о них сообщили
        открытые Telegram-каналы, и почти в реальном времени: карта
        обновляется, как только приходит сообщение. У каждого события
        видно, сколько независимых источников его подтвердили, и можно
...
def city_index_page(cities: list[dict], updated: str) -> str:
    return catalog_index_page(
        ...
        heading="Карта БПЛА и тревог по городам России",
        title="Карта БПЛА и воздушной тревоги по городам России онлайн",
        description=(
...
def district_index_page(districts: list[dict], updated: str) -> str:
    return catalog_index_page(
        ...
        heading="Карта БПЛА и тревог по районам России",
        title="Карта БПЛА и воздушной тревоги по районам России онлайн",
        description=(
