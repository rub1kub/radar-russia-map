...
def variants(name: str) -> list[str]:
    ...
    «С Азовского моря идут БПЛА», «в акватории Азовского моря», просто
    «Азовское» — падежи снимает стеммер, а вот сокращённую форму без слова
    «море» нужно положить рядом самим.
    """
    key = norm_key(name)
    short = key.replace(" море", "").strip()
    return [value for value in dict.fromkeys([key, short]) if len(value) > 3]
